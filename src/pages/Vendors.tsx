...
import { createParty, fetchVendors, type Party } from "../api/parties";
...
function emptyForm(): NewVendorForm {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{vendors.length} total</p>
          </div>
        ...
        {isFormOpen && (
          <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <h2 className="mb-4 text-lg font-semibold text-slate-900">New Vendor</h2>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {isLoading && (
            <div className="px-6 py-16 text-center text-sm text-slate-500">Loading vendors…</div>
          )}
              ...
              <tbody className="divide-y divide-slate-100">
                {vendors.map((vendor) => (
                  <tr key={vendor.id}>
                    ...
                    <td className="px-6 py-4 text-slate-600">{vendor.address || "—"}</td>
                    <td className="px-6 py-4 text-slate-600">{vendor.pan_number || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
