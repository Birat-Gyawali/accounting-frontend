...
import { fetchLiabilityAccounts, fetchExpenseAccounts, type Account } from "../api/accounts";
import { createBill, type CreateBillPayload } from "../api/bills";
...
export default function CreateBill() {
  ...
  useEffect(() => {
    Promise.all([fetchVendors(), fetchLiabilityAccounts(), fetchExpenseAccounts()])
      .then(([vendorsData, liabilityData, expenseData]) => {
        setVendors(vendorsData);
        setLiabilityAccounts(liabilityData);
  ...
  function validate(): string | null {
    ...
    for (const line of lines) {
      ...
      if (!line.expense_account) return "Every line needs an expense account.";
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-10 sm:px-8">
      <div className="mx-auto max-w-3xl">
        <h1 className="mb-6 text-2xl font-semibold text-slate-900">New Bill</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label htmlFor="bill_date" className="mb-1.5 block text-sm font-medium text-slate-700">
                Bill date
              </label>
              <input
                id="bill_date"
                ...
                <div key={line.key} className="rounded-xl border border-slate-200 p-4">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
                    ...
                    <div className="sm:col-span-3">
                      <label className="mb-1 block text-xs font-medium text-slate-500">Expense account</label>
                      <select
                        value={line.expense_account}
                        onChange={(e) => updateLine(line.key, { expense_account: e.target.value })}
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                      >
